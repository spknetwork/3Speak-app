...
class VideoTeaser extends Component<any,any> {
    constructor(props) {
        super(props);
        this.state = {
                    ...
                    <span>
                        ...
                        {(() => {
                            if(this.state.video_info.creation) {
                                const dateBest = convert(((new Date(new Date().toUTCString()) as any) / 1)  - (Date.parse(this.state.video_info.creation) / 1)).from("ms").toBest();
