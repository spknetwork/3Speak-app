...
class Vote extends React.Component<any,any> {
    ...
    async handleVote() {
        ...
        if (modalState === false) {
            ...
            if (profileID) {
                try {
                    const profile = await Utils.acctOps.getAccount(profileID) as any;
            ...
            <>
                <span className="ml-2 p-0">
                    ...
                    {(this.state.showModal) && (<span>
                        ...
                        <FontAwesomeIcon size={'lg'} icon={<FaChevronCircleUp style={{cursor: "pointer"}}/> as any} />
                        <FontAwesomeIcon size={'lg'} icon={<FaTimesCircle  style={{cursor: "pointer"}} className="text-danger"/> as any} />
                    </span>)}
                </span>
                ...
                <span className="ml-2 p-0">
                    ...
                    {(this.state.showDModal) && (<span>
                        ...
                        <FontAwesomeIcon size={'lg'} icon={<FaChevronCircleDown style={{cursor: "pointer"}}/> as any} />
                        <FontAwesomeIcon size={'lg'} icon={<FaTimesCircle  style={{cursor: "pointer"}} className="text-danger"/> as any} />
                    </span>)}
                </span>
                <OverlayTrigger rootClose trigger="click" placement="bottom" overlay={
