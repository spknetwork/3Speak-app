...
// hivesql variable not defined.  function not referenced - commenting out
// const hiveSQLQuery = (() => {
//     var stream = Pushable();
//     var run = async () => {
//         //await hivesql;
...
//     return stream;
// });
class GridFeed extends React.Component<any,any> {
    constructor(props) {
        super(props);
    ...
    async retrieveData() {
        ...
        if (this.props.type === "home") {
            fetch(`https://3speak.tv/apiv2/feeds/${this.props.type}`)
                ...
                .then(async json => {
                    for (var e in json) {
                        if (await PromiseIpc.send("blocklist.has", `hive:${json[e].author}:${json[e].permlink}` as any)) {
                            delete json[e];
                        }
                    }
                    console.log(json)
