...
class Pins extends Component<any,any> {
    pid: NodeJS.Timeout;
    constructor(props) {
        super(props);
        this.state = {
	...
	// type error - 2 arguments expected
        var pinls = await PromiseIpc.send("pins.ls", undefined as any)
        this.setState({
            pinls
        })
    }
    ...
    render() {
        ...
        for (const pin of this.state.pinls) {
			...
			// type error: date not compatible with arithmetic expression
                        return "In " + Math.round(Convert(pin.expire - (new Date().getTime())).from("ms").toBest().val) + Convert(pin.expire - (new Date().getTime())).from("ms").toBest().unit
                    })() : "Permanent"}
                </td>
                <td>
                    {pin.meta.pin_date ? (() => {
